        ...
        const double &timeOfFLight )
{
    ...
    Eigen::Vector6d fullCurrentCoefficients = bislip::Variables::computeFullCurrentCoefficients( bodyMap_, vehicleName_ );
   bislipSystems->setFullCurrentCoefficients( fullCurrentCoefficients );
...
*/
    ...
    if ( currentTrajectoryPhase == "Descent" )
    {
        ...
        greaterBankAngle = evaluatedBankAngle;
        ...
        if( debugInfo == 1 ){ std::cout << "                    Choose the greater of these to avoid skipping in the descent phase" << std::endl; }
    ...
    if( bislipSystems->getCurrentEngineStatus( ) == true )
    {
        ...
        double currentThrustMagnitude = bislip::Variables::computeThrustMagnitude( bodyMap_, vehicleName_ );
        bislipSystems->setCurrentThrustMagnitude( currentThrustMagnitude );
        if( debugInfo == 1 ){ std::cout << "    --------------------------" << std::endl; }
