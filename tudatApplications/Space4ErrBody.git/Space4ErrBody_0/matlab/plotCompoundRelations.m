...
for p = 1:numel(compilation)
    ...
    for k = 1:numel(compilation(p).evolutions)
        ...
        alphaMachBounds_Mach = compilation(1).evolutions(1).trajectories(1).individual.alphaMachBounds_Mach;
        alphaMachBounds_LB = compilation(1).evolutions(1).trajectories(1).individual.alphaMachBounds_LB;
        alphaMachBounds_UB = compilation(1).evolutions(1).trajectories(1).individual.alphaMachBounds_UB;
        
        plot(alphaMachBounds_Mach,alphaMachBounds_UB,'k','LineWidth',2);
        plot(alphaMachBounds_Mach,alphaMachBounds_LB,'k','LineWidth',2);
...
for p = 1:numel(compilation)
    ...
    for k = 1:numel(compilation(p).evolutions)
        ...
        stairs(compilation(1).evolutions(1).trajectories(1).individual.headingErrorDeadBand_distance,...
            compilation(p).evolutions(1).trajectories(1).individual.headingErrorDeadBand_LB,'k','LineWidth',2);
        stairs(compilation(1).evolutions(1).trajectories(1).individual.headingErrorDeadBand_distance,...
            compilation(1).evolutions(1).trajectories(1).individual.headingErrorDeadBand_UP,'k','LineWidth',2);
...
for p = 1:numel(compilation)
    ...
    if sum(abs(compilation(p).evolutions(k).trajectories(ii).individual.heading_error) > 30) == 0
                if compilation(p).evolutions(k).trajectories(ii).individual.distance_to_go(end) < 30
                    if sum(compilation(p).evolutions(k).trajectories(ii).individual.bank_angle_reversal_trigger) > 2
                        if compilation(p).evolutions(k).trajectories(ii).individual.E_hat(end) < 0.3                stairs(compilation(p).evolutions(k).trajectories(ii).individual.distance_to_go,...
                    compilation(p).evolutions(k).trajectories(ii).individual.evaluated_bank_angle);
                end
            end
                end
                ...
                if compilation(p).evolutions(k).trajectories(ii).individual.distance_to_go(end) < 30
                    if sum(compilation(p).evolutions(k).trajectories(ii).individual.bank_angle_reversal_trigger) > 2
                        if compilation(p).evolutions(k).trajectories(ii).individual.E_hat(end) < 0.3
                            ...
                            % semilogy(compilation(p).evolutions(k).trajectories(ii).individual.distance_to_go,...
